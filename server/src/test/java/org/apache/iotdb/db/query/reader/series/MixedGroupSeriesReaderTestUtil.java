...
public class MixedGroupSeriesReaderTestUtil {

  private static int seqFileNum = 5;
  private static int unseqFileNum = 0;
  private static int measurementNum = 5;
  private static int deviceNum = 5;
  private static long ptNum = 10;
  ...
  private static void prepareFiles(
    ...
    //      File file =
    //          new File(TestConstant.getTestTsFilePath(SERIES_READER_TEST_SG, 0, 0, i +
    // seqFileNum));
    //      TsFileResource tsFileResource = new TsFileResource(file);
    //      tsFileResource.setStatus(TsFileResourceStatus.CLOSED);
    ...
    //    File file =
    //        new File(
    //            TestConstant.getTestTsFilePath(SERIES_READER_TEST_SG, 0, 0, seqFileNum +
    // unseqFileNum));
    //    TsFileResource tsFileResource = new TsFileResource(file);
    //    tsFileResource.setStatus(TsFileResourceStatus.CLOSED);
  ...
  private static void prepareFile(
    ...
    for (String deviceId : deviceIds) {
      fileWriter.registerMixedGroup(deviceId, null, "template0");
    }
    for (long i = timeOffset; i < timeOffset + ptNum; i++) {
      for (String deviceId : deviceIds) {
        for (int subId = 0; subId < 5; subId++) {
          GroupRecord record = new GroupRecord(i, deviceId, (byte) subId);
          for (MeasurementSchema measurementSchema : measurementSchemas) {
            record.addTuple(
          ...
          fileWriter.writeMixedGroupRecord(record);
        }
