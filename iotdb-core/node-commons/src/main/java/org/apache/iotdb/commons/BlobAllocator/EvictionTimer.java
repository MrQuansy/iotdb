...
public class EvictionTimer {
          ...
          (PrivilegedAction<Void>)
              () -> {
                thread.setContextClassLoader(
                    EvictionTimer.EvictorThreadFactory.class.getClassLoader());
                return null;
              });
  ...
  private static class Reaper implements Runnable {
    ...
    public void run() {
      synchronized (EvictionTimer.class) {
        for (final Map.Entry<
                WeakReference<PoolArena.Evictor>, EvictionTimer.WeakRunner<PoolArena.Evictor>>
            entry : TASK_MAP.entrySet()) {
  ...
  private static final HashMap<
          WeakReference<PoolArena.Evictor>, EvictionTimer.WeakRunner<PoolArena.Evictor>>
      TASK_MAP = new HashMap<>(); // @GuardedBy("EvictionTimer.class")
  ...
  public static synchronized void cancel(
      final PoolArena.Evictor evictor, final Duration timeout, final boolean restarting) {
    if (evictor != null) {
      evictor.cancel();
      remove(evictor);
    }
   ...
   * @return the task map.
   */
  static HashMap<WeakReference<PoolArena.Evictor>, EvictionTimer.WeakRunner<PoolArena.Evictor>>
      getTaskMap() {
    return TASK_MAP;
  }
   ...
   */
  private static void remove(final PoolArena.Evictor evictor) {
    for (final Map.Entry<
            WeakReference<PoolArena.Evictor>, EvictionTimer.WeakRunner<PoolArena.Evictor>>
        entry : TASK_MAP.entrySet()) {
   ...
   * *must* call {@link #cancel(PoolArena.Evictor, Duration, boolean)} to cancel the task to prevent
  ...
  public static synchronized void schedule(
      final PoolArena.Evictor task, final Duration delay, final Duration period) {
    if (null == executor) {
      executor = new ScheduledThreadPoolExecutor(1, new EvictionTimer.EvictorThreadFactory());
      executor.setRemoveOnCancelPolicy(true);
      ScheduledExecutorUtil.safelyScheduleAtFixedRate(
          executor,
          new EvictionTimer.Reaper(),
          delay.toMillis(),
          period.toMillis(),
          TimeUnit.MILLISECONDS);
    }
    final WeakReference<PoolArena.Evictor> ref = new WeakReference<>(task);
    final EvictionTimer.WeakRunner<PoolArena.Evictor> runner = new EvictionTimer.WeakRunner<>(ref);
